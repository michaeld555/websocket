...
io.on('connection', function(socket) {
    ...
    socket.on('init', function(init) {
		...
		if (init.logged) {
			console.log('SELECT * FROM `users` WHERE `steamid`=' + connection.escape(init.steamid) + ' AND `token_time`=' + connection.escape(init.time) + ' AND `token`=' + connection.escape(init.token) + ' LIMIT 1')
